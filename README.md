# @lthn/sdk-typescript-axios

axios client — Node + browser, the de-facto ergonomic REST shape.

Auto-generated from the lthn OpenAPI spec at:

  https://github.com/LetheanNetwork/desktop/blob/main/build/sdk/openapi.yaml

Do **not** hand-edit. Regenerate via:

  ```bash
  cd /path/to/desktop
  ./build/sdk/publish.sh typescript-axios
  ```

## Install

```bash
npm install @lthn/sdk-typescript-axios
```

## License

EUPL-1.2 — same as the upstream lthn project.
